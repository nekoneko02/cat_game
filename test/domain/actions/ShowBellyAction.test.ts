import { ShowBellyAction } from '@/domain/actions/ShowBellyAction';
...
describe('ShowBellyAction', () => {
  ...
  describe('getInternalStateChange', () => {
    ...
    it('3秒間の合計で意図した値になる', () => {
      ...
      expect(change.fear * duration).toBeCloseTo(-0.2, 2);
    });

    it('bondingの変化量が最も大きい', () => {
      const change = action.getInternalStateChange();

      expect(change.bonding).toBeGreaterThan(change.playfulness);
      expect(Math.abs(change.fear)).toBeGreaterThan(change.playfulness);
    });
  });
...
describe('ShowBellyAction Integration Tests', () => {
  ...
  it('シナリオ: createActionResultで完全な結果を取得', () => {
    ...
    expect(actionResult.internalStateChange?.fear).toBe(-0.067);
